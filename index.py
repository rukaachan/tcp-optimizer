...
def run_profile_benchmark(stdscr, profile_key):
    ...
    before_params = {p: su.get_sysctl_value(p) for p in key_params_to_check if su.get_sysctl_value(p)}
    ...
    if PROFILES is None or profile_key not in PROFILES:
        ...
        return
    su.write_sysctl_config(PROFILES[profile_key]["settings"])
    su.apply_sysctl_from_conf()
    after_params = {p: su.get_sysctl_value(p) for p in key_params_to_check if su.get_sysctl_value(p)}
